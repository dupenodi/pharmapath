...
from app.agent.openai_loop import run_openai_turn
...
async def run_agent_turn(graph: nx.MultiDiGraph, session_id: str, message: str) -> dict:
    ...
    All three providers share the same 8 tools, system prompt, and
    ...
    if settings.agent_provider == "openai":
        return await run_openai_turn(graph, session_id, message)
